...
#In this script, I generate PCAs from ATAC-seq peaks of young cultured qNSC/aNSCs as well as PCAs comparing in vitro vs in vivo ATAC-seq peaks
...
consensus_count_matrix <- read.table("Original_Data/FINAL_PEAKSETS_COUNT_MATRICES/inVitro/invitro_consensus_countMatrix_Raw.txt", header=TRUE, sep="\t") 
...
pdf("Output_Figs/InVitro_Y_O_Q_A_PC1_PC2.pdf",height=10,width=12.5)
ggplot(pca.vst.df, aes(PC1,PC2, color=pca.vst.df$Group)) +
  geom_point(size=12, shape=18)+
  scale_color_manual(values=c("lightskyblue","orange","dodgerblue","red2"),breaks=c("Y_qNSC","Y_aNSC","O_qNSC","O_aNSC"),labels=c("Young qNSC","Young aNSC/NPC","Old qNSC","Old aNSC/NPC"), name="")+
...
pdf("Output_Figs/InVitro_Y_O_Q_A_PC1_PC3.pdf",height=10,width=12.5)
...
pdf("Output_Figs/InVitro_Y_O_Q_A_PC1_PC4.pdf",height=11.5,width=16)
...
######################################################################################################
#Visualizing qNSC and aNSC libraries from both in vivo and in vitro experiments on the same PCA
######################################################################################################
rm(list=ls())
library("DESeq2")
library("ggplot2")
...
consensus_count_matrix <- read.table("Original_Data/FINAL_PEAKSETS_COUNT_MATRICES/inVitro/Combined_InVitro_InVivo_countMatrix_Raw.txt", header=TRUE, sep="\t")
...
sampleTable$Condition <- c(rep(c("In Vivo"),11),rep(c("Cultured"),10))
str(sampleTable)
...
pdf("Output_Figs/Combined_InVivo_InVitro_Y_O_Q_A_PC1_PC2.pdf",height=10,width=12.5)
ggplot(pca.vst.df, aes(PC1,PC2, color=Cell_Type, shape=Age, fill=factor(ifelse(Condition=="Cultured",Cell_Type,"white")))) +
  ...
  scale_size_manual(name = "Condition", breaks=c("In Vivo", "Cultured"),values = c(9, 9.01))+
